...
import com.mes.code.server.service.po.aps.APSCondition;
...
public class APSConditionDAO extends BaseDAO {
	...
	public int Update(BMSEmployee wLoginUser, APSCondition wAPSCondition, OutResult<Integer> wErrorCode) {
		...
		try {
			...
			if (wAPSCondition.getID() <= 0) {
				wSQL = MessageFormat.format("INSERT INTO {0}.aps_condition(OMSOrderPrioritys,LineOrders,"
				...
				wSQL = MessageFormat.format("UPDATE {0}.aps_condition SET OMSOrderPrioritys = :OMSOrderPrioritys,"
	...
	public ServiceResult<Integer> DeleteList(BMSEmployee wLoginUser, List<APSCondition> wList,
		...
		try {
			...
			String wSql = MessageFormat.format("delete from {1}.aps_condition WHERE ID IN({0}) ;",
	...
	public List<APSCondition> SelectList(BMSEmployee wLoginUser, int wID, int wShiftPeriod,
		...
		try {
			...
			String wSQL = MessageFormat.format("SELECT * FROM {0}.aps_condition WHERE  1=1  "
